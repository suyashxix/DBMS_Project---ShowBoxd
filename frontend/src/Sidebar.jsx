...
const styles = `
  ...
  @keyframes sb-slide-out { to { transform: translateX(-100%); } }
  @keyframes sb-overlay-out { to { opacity: 0; } }
  ...
  .sb-brand-icon svg { width: 22px; height: 22px; color: #111; }
  .sb-brand-name {
  ...
  .sb-close {
    ...
    border: 1px solid #333; background: #1f1f1f; color: #888;
  ...
  .sb-content { padding: 12px 0 40px; flex: 1; }

  .sb-section { padding: 24px 0 12px; border-bottom: 1px solid #1e1e1e; }
  ...
  .sb-section-icon { width: 22px; height: 22px; color: #d4a853; }
  .sb-section-title {
    font-family: 'DM Serif Display', serif;
  ...
  .sb-item {
    ...
    font-family: 'DM Sans', sans-serif; font-size: 15px; color: #999;
  ...
  .sb-genre-pill:hover { border-color: #d4a853; color: #d4a853; background: rgba(212,168,83,0.1); }
      ...
      <div className="sb-panel" ref={panelRef}>
        ...
        <div className="sb-header">
          <div className="sb-brand">
            <div className="sb-brand-icon"><FilmIcon /></div>
            ...
            <div className="sb-genres">
              {GENRES.map((g, i) => (
                <button
                  ...
                  onClick={() => go(`/home?genre=${encodeURIComponent(g)}`)}  // ← fixed: was /?genre=
                >
                  {g}
                </button>
              ))}
