...
const styles = `
  ...
  .tv-banner-arrow:hover {
    background: rgba(255,255,255,0.3);
  }
  .tv-banner-arrow.left { left: 24px; }
  .tv-banner-arrow.right { right: 24px; }
  ...
  .tv-airing-dot {
    ...
    border-radius: 50%;
    background: #fff;
    animation: tv-pulse 1.4s ease infinite;
  }
...
export default function TVShowsPage() {
  ...
  useEffect(() => {
    axios.get(`${API}/api/trending/`)
        .then(r => {
          ...
          setTrending(d.filter(i => i.media_type === 'tv_show').slice(0, 14));
        })
        .catch(() => setTrending([]))
  ...
  useEffect(() => {
    ...
    axios.get(`${API}/api/user/${user.user_id}/recommendations/`)
        .then(r => {
          const d = Array.isArray(r.data) ? r.data : (r.data.results ?? []);
          setRecommended(d.filter(i => i.media_type === 'tv_show').slice(0, 14));
        })
    ...
    axios.get(`${API}/api/search/`, {
      params: { type: 'tv_show' }
        ...
        .then(res => {
          ...
          setBannerShows(sorted.slice(0, 5));
        })
        .catch(() => setBannerShows([]));
  }, []);
  ...
  const prevSlide = () => {
    if (bannerShows.length === 0) return;
    setSlide((p) => (p - 1 + bannerShows.length) % bannerShows.length);
  };

  const nextSlide = () => {
    if (bannerShows.length === 0) return;
    setSlide((p) => (p + 1) % bannerShows.length);
  };
        ...
        <div className="tv-wrap">
          ...
          <div className="tv-banner">
            {bannerShows.length > 0 ? (
                <>
                  ...
                  <button className="tv-banner-arrow left" onClick={prevSlide}>
                    <ChevLeft />
                  </button>
                  <button className="tv-banner-arrow right" onClick={nextSlide}>
                    <ChevRight />
                  </button>
                </>
            ) : (
          ...
          <div className="tv-container">
            ...
            <ScrollRow
                ...
                linkTo="/home?top_rated=true&type=tv_show"
            />
