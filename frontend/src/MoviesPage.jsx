...
const styles = `
  ...
  .mp-banner-rating {
    ...
    font-size: 15px; color: #d4a853; font-weight: 500;
  }
  .mp-banner-genre {
  ...
  .mp-banner-btn.primary {
    background: #d4a853; color: #111;
  }
  .mp-banner-btn.primary:hover { background: #c49840; transform: translateY(-1px); }
  .mp-banner-btn.secondary {
    background: rgba(255,255,255,0.12); color: #fff;
    backdrop-filter: blur(8px);
    border: 1px solid rgba(255,255,255,0.2);
  }
  .mp-banner-btn.secondary:hover { background: rgba(255,255,255,0.2); }
  ...
  .mp-banner-arrow:hover { background: rgba(255,255,255,0.22); }
  ...
  .mp-section-title {
    font-family: 'DM Serif Display', serif;
    font-size: 30px; font-weight: 400; color: #111;
  }
  .mp-section-link {
    font-size: 13px; font-weight: 500;
  ...
  .mp-scroll-btn {
    ...
    border: 1px solid #e4e0d8; color: #111;
  ...
  /* Poster card */
  .mp-card {
    flex: 0 0 190px; text-decoration: none;
  ...
  .mp-card-hover-rating {
    display: flex; align-items: center; gap: 4px;
    font-size: 14px; font-weight: 600; color: #d4a853;
    margin-bottom: 6px;
  }
  .mp-card-hover-btn {
  ...
  /* Wide card (for new releases) */
  .mp-wide-card {
    flex: 0 0 260px; text-decoration: none;
    color: inherit; transition: transform 0.22s;
  }
  .mp-wide-card:hover { transform: translateY(-4px); }
  .mp-wide-poster {
    width: 260px; height: 150px; border-radius: 10px;
    object-fit: cover; background: #e4e0d8;
  ...
  .mp-empty { text-align: center; padding: 48px; color: #aaa; font-size: 15px; }
  .mp-loading { text-align: center; padding: 48px; color: #bbb; }
  .mp-spinner {
  ...
  .mp-ribbon-dot {
    ...
    background: #fff; animation: mp-pulse 1.4s ease infinite;
  }
...
const GENRE_COLORS = {
  Action:    ['#c0392b','#e74c3c'], Comedy:   ['#f39c12','#f1c40f'],
  Drama:     ['#2c3e50','#34495e'], Thriller: ['#1a1a2e','#16213e'],
  Horror:    ['#1a0a0a','#4a0a0a'], Romance:  ['#c0392b','#e91e8c'],
  'Sci-Fi':  ['#0d47a1','#1565c0'], Animation:['#6a1b9a','#9c27b0'],
  Documentary:['#1b5e20','#2e7d32'],Fantasy:  ['#4a148c','#7b1fa2'],
  Crime:     ['#212121','#424242'], Adventure:['#e65100','#f57c00'],
  Mystery:   ['#1a237e','#283593'], Biography:['#37474f','#546e7a'],
};
...
export default function MoviesPage() {
  ...
  const [loading, setLoading] = useState({
    ...
    nowShowing: true, newReleases: true, recommended: false,
    genre: false,
  });
  ...
  useEffect(() => {
    axios.get(`${API}/api/trending/`)
      .then(r => { const d = Array.isArray(r.data) ? r.data : (r.data.results ?? []); setTrending(d.filter(i => i.media_type === 'movie').slice(0, 14)); })
      .catch(() => setTrending([]))
  ...
  useEffect(() => {
    axios.get(`${API}/api/nowshowing/`)
      .then(r => { const d = Array.isArray(r.data) ? r.data : (r.data.results ?? []); setNowShowing(d.slice(0, 14).map(i => ({...i, nowShowing: true}))); })
      .catch(() => setNowShowing([]))
  ...
  useEffect(() => {
    ...
    axios.get(`${API}/api/user/${user.user_id}/recommendations/`)
      .then(r => { const d = Array.isArray(r.data) ? r.data : (r.data.results ?? []); setRecommended(d.filter(i => i.media_type === 'movie').slice(0, 14)); })
      .catch(() => setRecommended([]))
      .finally(() => setL('recommended', false));
  }, [user]);

  // Banner: pick from popular + topRated combined, deduplicated
  useEffect(() => {
    const combined = [...popular, ...topRated].reduce((acc, item) => {
      if (!acc.find(x => x.media_id === item.media_id)) acc.push(item);
      return acc;
    }, []);
    setBannerMovies(combined.slice(0, 5));
  }, [popular, topRated]);

  // Genre filtering
  const handleGenre = (genre) => {
    if (activeGenre === genre) { setActiveGenre(null); setGenreMovies([]); return; }
    setActiveGenre(genre);
    setL('genre', true);
    axios.get(`${API}/api/search/`, { params: { type: 'movie', genre } })
      .then(r => { const d = Array.isArray(r.data) ? r.data : (r.data.results ?? []); setGenreMovies(d.slice(0, 14)); })
  ...
  const current = bannerMovies[slide];
                ...
                {bannerMovies.map((m, i) => (
                  <div className="mp-banner-slide" key={m.media_id}>
                    ...
                    <div className="mp-banner-content">
                      <div className="mp-banner-eyebrow">
                        <span className="mp-banner-tag">🎬 Featured Film</span>
                        <span className="mp-banner-num">{String(i+1).padStart(2,'0')} / {String(bannerMovies.length).padStart(2,'0')}</span>
                      </div>
